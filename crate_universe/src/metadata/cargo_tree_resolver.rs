...
impl TreeResolver {
    ...
    fn copy_project_with_explicit_deps_on_all_transitive_proc_macros(
        ...
        let output_dir_tmp = output_dir.join(root.as_std_path());
        let output_dir = output_dir_tmp.as_path();
        ...
        for file in std::fs::read_dir(pristine_root).context("Failed to read dir")? {
            ...
            if file_name != "Cargo.toml" && file_name != "Cargo.lock" {
                ...
                println!("XXX {:?} --> {:?}", source_path, destination);
        ...
        for member_dir in members {
            ...
            println!("XXX ml {:?} --> {:?}", source_path, destination);
